  ...
  <div class="cont">
    ...
    <dl class="helpNav vipNav">
     <dt>我的主页</dt>
      <dd>
       ...
       <a href="vipshopcartinfo?accountId=<%=s.getAttribute("member")%>" class="vipCur">我的收藏</a>
   ...
   <div class="contRight">
    ...
    <tr>
     ...
     <td><strong class="red">${pageInfo.goods.goodsPrice}</strong></td>
     <td><strong >${pageInfo.goods.tradeMode}</strong></td>

     <td><a href="shopinfo?goodsId=${pageInfo.goods.goodsId}" class="green">查看</a><br />
         <a href="deleteshop?goodsId=${pageInfo.goods.goodsId}" class="green">删除</a></td>
    </tr>
    </c:forEach>
   </table><!--orderList/-->
    <br/>
   ...
   <!-- 显示分页信息块 -->
					<div class="scott">
						...
						<br>
						<!-- 显示分页条 -->
						<a href="vipshopcartinfo?accountId=<%=s.getAttribute("member")%>&pn=1">首页</a>
						<c:if test="${pageInfo.hasPreviousPage}">
						<a href="vipshopcartinfo?accountId=<%=s.getAttribute("member")%>&pn=${pageInfo.pageNum-1}">&laquo;</a>
						</c:if> 
						<c:forEach items="${pageInfo.navigatepageNums}" var="pageNum" >	
						<a href="vipshopcartinfo?accountId=<%=s.getAttribute("member")%>&pn=${pageNum}">${pageNum}</a> 
						</c:forEach>
						<c:if test="${pageInfo.hasNextPage}">
						<a href="vipshopcartinfo?accountId=<%=s.getAttribute("member")%>&pn=${pageInfo.pageNum+1}">&raquo;</a>
						</c:if>  
						<a href="vipshopcartinfo?accountId=<%=s.getAttribute("member")%>&pn=${pageInfo.pages}">尾页</a>
					</div>
					    <br/>
  </div><!--cont/-->
  <div class="inHelp">

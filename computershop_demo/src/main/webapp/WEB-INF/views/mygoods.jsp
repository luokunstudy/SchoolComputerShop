  ...
  <div class="cont">
    ...
    <dl class="helpNav vipNav">
     <dt>我的主页</dt>
      <dd>
       ...
       <a href="Myshoping?account=<%=s.getAttribute("member")%>" class="vipCur">我的商品</a>
   ...
   <div class="contRight">
    ...
    <tr>
     <td>
      <dl>
       <dt><a href="#"><img src="<%=basePath%>upload/${pageInfo.goodsPhone}" width="85" height="85" /></a></dt>
       <dd>${pageInfo.goodsName}<br /></dd>
      </dl>
     </td>
     <td><strong class="red">${pageInfo.goodsPrice}</strong></td>
     <td><strong >${pageInfo.tradeMode}</strong></td>
     <td><a href="Apply?goodsId=${pageInfo.goodsId}" class="green">申请</a><br />
         <a href="deleteshop?goodsId=${pageInfo.goodsId}&account=<%=s.getAttribute("member")%>" class="green">下架</a></td>
    </tr>
    </c:forEach>
   </table><!--orderList/-->
    <br/>
   ...
   <!-- 显示分页信息块 -->
					<div class="scott">
						...
						<br>
						<!-- 显示分页条 -->
						<a href="Myshoping?account=<%=s.getAttribute("member")%>&pn=1">首页</a>
						<c:if test="${pageInfo.hasPreviousPage}">
						<a href="Myshoping?account=<%=s.getAttribute("member")%>&pn=${pageInfo.pageNum-1}">&laquo;</a>
						</c:if> 
						<c:forEach items="${pageInfo.navigatepageNums}" var="pageNum" >	
						<a href="Myshoping?account=<%=s.getAttribute("member")%>&pn=${pageNum}">${pageNum}</a> 
						</c:forEach>
						<c:if test="${pageInfo.hasNextPage}">
						<a href="Myshoping?account=<%=s.getAttribute("member")%>&pn=${pageInfo.pageNum+1}">&raquo;</a>
						</c:if>  
						<a href="Myshoping?account=<%=s.getAttribute("member")%>&pn=${pageInfo.pages}">尾页</a>
					</div>
                   <br>
  </div><!--cont/-->
  <div class="inHelp">
